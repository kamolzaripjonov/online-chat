    ...
    <div className="fixed inset-0 bg-slate-900 dark:bg-slate-100 flex flex-col z-50">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        {previewUrl ? (
          ...
          >
            <div className={`w-20 h-20 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-slate-600' : 'bg-gray-200'}`}>
              <Camera className={`w-10 h-10 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`} />
            </div>
            <p className={`font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Add to your story</p>
            ...
            <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Tap to select from gallery</p>
          </button>
        )}
      </div>
    </div>
