...
const DEMO_USERS = [
    {
        ...
        full_name: 'Sarah Photography',
        avatar_url: 'https://api.dicebear.com/7.x/avataaars/svg?seed=sarah'
    },
    {
        user_id: 'demo-2',
        username: 'alex_daily',
        full_name: 'Alex Daily',
        avatar_url: 'https://api.dicebear.com/7.x/avataaars/svg?seed=alex'
    },
    {
        user_id: 'demo-3',
        username: 'mike_films',
        full_name: 'Mike Films',
        avatar_url: 'https://api.dicebear.com/7.x/avataaars/svg?seed=mike'
    },
    {
        user_id: 'demo-4',
        username: 'emma_design',
        full_name: 'Emma Design',
...
export default function ChatPage({onStartCall, hideNavigation, showNavigation}) {
    ...
    const loadConversations = () => {
        ...
        const enrichedConvs = convs.map(conv => {
            ...
            const otherUser = DEMO_USERS.find(u => u.user_id === otherId) || {
                username: 'unknown',
                full_name: 'Unknown',
    ...
    const sendMessage = (e) => {
        ...
        if (messageInput.trim() === lastMessage) {
            setMessageInput('');
            return;
        }
        ...
        if (convIndex !== -1) {
            convs[convIndex].last_message = {content: messageInput.trim()};
            ...
            <div className={`fixed inset-0 flex flex-col ${isDarkMode ? 'bg-slate-900' : 'bg-gray-50'} z-50`}>
                <div
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        <img src={selectedUserProfile?.avatar_url} alt=""
                        ...
                        {messages.length === 0 && (
                            <div className="text-center py-16">
                                ...
                                <p className={`text-sm mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Start a
                                    conversation with {selectedUserProfile?.username}</p>
                            </div>
                        )}
            ...
            <div className={`sticky top-0 z-10 p-4 ${isDarkMode ? 'bg-slate-900' : 'bg-gray-50'}`}>
                ...
                {searchResults.length > 0 && (
                    <div
                        ...
                        {searchResults.map((p) => (
                            <button key={p.user_id} onClick={() => startConversation(p.user_id)}
