...
import {messageService, userService} from '../service/authService';
import {Search, Send, Phone, Video, ArrowLeft, Image, Info} from 'lucide-react';
...
export default function ChatPage({onStartCall, hideNavigation, showNavigation}) {
    ...
    const loadConversations = async () => {
        try {
            // Get users and load recent messages
            const users = await userService.searchUsers('');
            setConversations(users.slice(0, 10) || []);
        } catch (err) {
    ...
    const loadMessages = async (otherUserId) => {
        try {
            setLoading(true);
            const msgs = await messageService.getMessages(otherUserId, 50);
            setMessages(msgs || []);
        } catch (err) {
    ...
    const sendMessage = async (e) => {
        ...
        try {
            await messageService.sendMessage(selectedConversation, messageInput.trim(), 'text');
            setMessageInput('');
            loadMessages(selectedConversation);
            loadConversations();
        } catch (err) {
    ...
    const selectedUser = conversations.find(u => u._id === selectedConversation);
                ...
                <div
                    ...
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                        ...
                        <div className="min-w-0">
                            <p className={`font-semibold truncate ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedUser?.name || selectedUser?.username}</p>
                    ...
                    <div className="max-w-2xl mx-auto space-y-1.5">
                        {loading ? (
                            ...
                            <div className="text-center py-16">
                                ...
                                <p className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedUser?.name || selectedUser?.username}</p>
                            ...
                            messages.map((msg) => (
                                <div key={msg._id}
                                    ...
                                    <div
                                        className={`max-w-[80%] sm:max-w-[70%] px-3.5 py-2 rounded-2xl ${msg.sender._id === user?.id
                                        ...
                                        <p className={`text-[10px] mt-1 text-right ${msg.sender._id === user?.id ? 'text-blue-200' : isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>
                        ...
                        {searchResults.map((user) => (
                            <button key={user._id} onClick={() => startConversation(user._id)}
                                ...
                                <div className="text-left">
                                    <p className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{user.name || user.username}</p>
                    ...
                    conversations.map((conv) => (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0 text-left">
                                <p className={`font-semibold truncate ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{conv.name || conv.username}</p>
                                <div className="flex items-center gap-1">
                                    <p className={`text-sm truncate ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Last
                                        message...</p>
                                </div>
                            </div>
