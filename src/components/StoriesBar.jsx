            ...
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                {user && !hasUserStory && (
                    <button className="flex flex-col items-center gap-1 min-w-[70px] group flex-shrink-0">
                        <div
                            className={`relative w-16 h-16 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'}`}>
                            <Plus className={`w-8 h-8 ${isDarkMode ? 'text-white' : 'text-gray-700'}`}/>
                        </div>
                        <span
                            className={`text-xs truncate max-w-[60px] ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {t('stories')}
            </span>
                    </button>
                )}
                ...
                {storyGroups.map((group) => (
                    ...
                    >
                        <div
                            className={`w-16 h-16 rounded-full p-0.5 ${group.has_unviewed ? 'bg-gradient-to-tr from-blue-500 via-purple-500 to-pink-500' : isDarkMode ? 'bg-slate-600' : 'bg-gray-300'}`}>
                            <img
