...
export default function PostCard({ post, onRefresh }) {
  ...
  const handleComment = (e) => {
    ...
    if (commentInput.trim() === lastComment) { setCommentInput(''); return; }
    setSubmitting(true);
    const newComment = {
      id: `comment-${Date.now()}`,
      post_id: post.id,
  ...
  return (
    <article className={`${isDarkMode ? 'bg-slate-900 border-b border-slate-800 sm:rounded-xl sm:border' : 'bg-white border-b border-gray-100 sm:rounded-xl sm:border sm:shadow-sm'}`}>
      {/* Header */}
      <div className="flex items-center justify-between p-3">
        <div className="flex items-center gap-3">
          <img src={post.profiles.avatar_url} alt="" className="w-9 h-9 sm:w-10 sm:h-10 rounded-full object-cover" />
          <div>
        ...
        <button className={`p-2 rounded-full transition ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>•••</button>
      </div>
      ...
      <div className="flex items-center justify-between px-3 py-2">
        <div className="flex items-center gap-4">
          <button onClick={handleLike} className={`transition touch-manipulation ${isLiked ? 'text-red-500' : isDarkMode ? 'text-white' : 'text-gray-900'}`}>
            <Heart className={`w-7 h-7 ${isLiked ? 'fill-current' : ''}`} />
          </button>
          <button onClick={() => setShowComments(!showComments)} className={`transition touch-manipulation ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
      ...
      {showComments && (
        <div className={`border-t ${isDarkMode ? 'border-slate-800' : 'border-gray-100'}`}>
                  ...
                  <div className="flex gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <button onClick={() => setReplyingTo(comment.id)} className="text-xs font-medium text-gray-500 hover:text-gray-700 transition">{t('reply')}</button>
                      </div>
                      {comment.replies && comment.replies.length > 0 && (
                        <div className="mt-2 ml-2 space-y-2">
                          {comment.replies.map((reply) => (
                            <div key={reply.id} className="flex gap-2">
                              <img src={reply.profiles.avatar_url} alt="" className="w-6 h-6 rounded-full flex-shrink-0" />
                              <p className={`text-sm ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                                <span className="font-semibold">{reply.profiles.username}</span>{' '}
                                {reply.content}
                                <span className="text-xs text-gray-500 ml-2">{timeAgo(reply.created_at)}</span>
                              </p>
                            </div>
                          ))}
                        </div>
                        ...
                        <form onSubmit={(e) => handleReply(e, comment.id, comment.user_id)} className="mt-2">
                          <div className="flex gap-2">
                            <input
                              ...
                              autoFocus
                            />
                            <button type="submit" disabled={!replyInput.trim() || submitting} className="text-blue-500 font-semibold text-sm disabled:opacity-50">Post</button>
                          </div>
                        </form>
                      )}
                    </div>
          ...
          <form onSubmit={handleComment} className={`flex gap-2 p-3 border-t ${isDarkMode ? 'border-slate-800' : 'border-gray-100'}`}>
            <input
              type="text"
              value={commentInput}
            ...
            />
            <button type="submit" disabled={!commentInput.trim() || submitting} className="text-blue-500 font-semibold text-sm disabled:opacity-50">Post</button>
          </form>
        </div>
      )}
    </article>
