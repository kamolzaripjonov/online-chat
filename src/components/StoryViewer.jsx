    ...
    return (
        <div className="fixed inset-0 bg-black z-50" ref={containerRef}>
            <div className="absolute top-4 left-4 right-4 flex gap-1 z-10">
                {stories.map((_, index) => (
                    <div key={index} className="h-1 flex-1 bg-gray-800 rounded overflow-hidden">
                        <div data-story-index={index}
                             className={`h-full bg-white transition-all ${index < currentIndex ? 'w-full' : index > currentIndex ? 'w-0' : ''}`}/>
                    </div>
                ))}
            </div>
            ...
            <div className="absolute bottom-8 left-4 right-4 flex items-center gap-4 z-10">
                <div
                    className={`flex-1 flex items-center gap-3 rounded-full px-4 py-2 ${isDarkMode ? 'bg-black/30' : 'bg-white/20'}`}>
                    <input type="text" placeholder={t('typeMessage').replace('...', '')}
